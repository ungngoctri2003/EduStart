...
import { Eye, GraduationCap, ListChecks, ListVideo, Pencil, PlusCircle, Trash2, UserPlus } from 'lucide-react';
...
const ADMIN_TAB_KEYS = ['users', 'courses', 'payments', 'content'];
...
export function DashboardAdmin() {
  ...
  const tab = useMemo(() => {
    ...
    if (t === 'enrollments' || t === 'quiz_stats' || t === 'classes') return 'courses';
    ...
    (v) => {
      ...
      setSearchParams(
        (prev) => {
          ...
          if (v === 'payments') {
            if (!next.get('paymentsArea')) next.set('paymentsArea', 'classes');
          } else {
  ...
  useEffect(() => {
    const t = searchParams.get('tab');
    const legacyTab = t === 'enrollments' || t === 'quiz_stats' || t === 'classes';
    if (!legacyTab && !searchParams.get('courseArea')) return;
    setSearchParams(
      (prev) => {
        const next = new URLSearchParams(prev);
  ...
  useEffect(() => {
    if (!classDetailRow?.id || tab !== 'courses') return;
  ...
  async function submitClassForm(e) {
    ...
    if (!classForm.course_id) {
      toast.error(DASH_ADMIN.CLASS_COURSE_REQUIRED);
      return;
    }
    try {
      if (!classEditingId) {
    ...
    <>
                  ...
                  <AdminDataTable>
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell sx={adminHeaderCellSx}>{DASH_ADMIN.TH_COURSE}</TableCell>
                    ...
                    <TableBody>
                      {classesItems.map((row) => (
                        <TableRow key={row.id} sx={adminBodyRowSx}>
                          ...
                          <TableCell>{row.course?.title || row.course_id || '—'}</TableCell>
                          <TableCell>
      ...
      >
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, pt: 3 }}>
          ...
          />
          <FormControl size="small" fullWidth>
            <InputLabel id="class-form-course">{DASH_ADMIN.TH_COURSE}</InputLabel>
            <Select
              labelId="class-form-course"
              label={DASH_ADMIN.TH_COURSE}
