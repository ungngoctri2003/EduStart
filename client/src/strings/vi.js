/** Chuoi giao dien tieng Viet (Unicode escape, file ASCII). */

export const NAV = {
  OPEN_MENU: 'M\u1EDF menu',
  CLOSE_MENU: '\u0110\u00F3ng menu',
  HOME: 'Trang ch\u1EE7',
  ABOUT: 'Gi\u1EDBi thi\u1EC7u',
  COURSES: 'Kh\u00F3a h\u1ECDc',
  TEAM: '\u0110\u1ED9i ng\u0169',
  TESTIMONIALS: '\u0110\u00E1nh gi\u00E1',
  CONTACT: 'Li\u00EAn h\u1EC7',
  DASHBOARD: 'B\u1EA3ng \u0111i\u1EC1u khi\u1EC3n',
  SIGN_OUT: '\u0110\u0103ng xu\u1EA5t',
  SIGN_IN: '\u0110\u0103ng nh\u1EADp',
};

export const FOOTER = {
  QUICK_LINKS: 'Li\u00EAn k\u1EBFt nhanh',
  PRIVACY: 'Ch\u00EDnh s\u00E1ch b\u1EA3o m\u1EADt',
  TERMS: '\u0110i\u1EC1u kho\u1EA3n s\u1EED d\u1EE5ng',
  CONTACT_TITLE: 'Li\u00EAn h\u1EC7',
  ADDRESS: '123 \u0110\u01B0\u1EDDng ABC, Bangalore, Karnataka',
  SHARE: 'Chia s\u1EBB',
  COMMUNITY: 'C\u1ED9ng \u0111\u1ED3ng',
  VIDEO: 'Video',
  LINK: 'Li\u00EAn k\u1EBFt',
  NEWSLETTER: 'B\u1EA3n tin',
  NEWSLETTER_DESC:
    '\u0110\u0103ng k\u00FD \u0111\u1EC3 nh\u1EADn tin kh\u00F3a h\u1ECDc v\u00E0 c\u1EADp nh\u1EADt m\u1EDBi.',
  EMAIL_PLACEHOLDER: 'Email c\u1EE7a b\u1EA1n',
  SUBSCRIBE: '\u0110\u0103ng k\u00FD',
  COPYRIGHT: 'B\u1EA3o l\u01B0u m\u1ECDi quy\u1EC1n.',
};

export const PAGE = {
  HOME_CRUMB: 'Trang ch\u1EE7',
};

export const LAYOUT = {
  BACK_TO_TOP: 'L\xEAn \u0111\u1EA7u trang',
};

export const ROLE = {
  NO_PROFILE:
    'T\xE0i kho\u1EA3n ch\u01B0a c\xF3 h\u1ED3 s\u01A1. H\xE3y \u0111\u0103ng xu\u1EA5t v\xE0 \u0111\u0103ng nh\u1EADp l\u1EA1i, ho\u1EB7c li\xEAn h\u1EC7 h\u1ED7 tr\u1EE3.',
};

export const ERR = {
  LOAD: 'Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c d\u1EEF li\u1EC7u',
  LOAD_ENROLLMENTS: 'Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c \u0111\u0103ng k\xFD kh\xF3a h\u1ECDc',
  LOAD_TEAM: 'Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c \u0111\u1ED9i ng\u0169',
  LOAD_TESTIMONIALS: 'Kh\xF4ng t\u1EA3i \u0111\u01B0\u1EE3c \u0111\xE1nh gi\xE1',
  LOAD_FAILED: 'T\u1EA3i th\u1EA5t b\u1EA1i',
  NOT_FOUND: 'Kh\xF4ng t\xECm th\u1EA5y',
  LOGIN_FAILED: '\u0110\u0103ng nh\u1EADp th\u1EA5t b\u1EA1i',
  SIGNUP_FAILED: '\u0110\u0103ng k\xFD th\u1EA5t b\u1EA1i',
  SEND_FAILED: 'G\u1EEDi th\u1EA5t b\u1EA1i',
  ENROLL_FAILED: '\u0110\u0103ng k\xFD kh\xF3a h\u1ECDc th\u1EA5t b\u1EA1i',
  INVALID_JSON: 'JSON kh\xF4ng h\u1EE3p l\u1EC7',
};

export const COMMON = {
  LOADING: '\u0110ang t\u1EA3i\u2026',
  PLEASE_WAIT: 'Vui l\xF2ng ch\u1EDD\u2026',
  EMAIL: 'Email',
  PASSWORD: 'M\u1EADt kh\u1EA9u',
  DELETE: 'X\xF3a',
  ADD: 'Th\xEAm',
  SAVE: 'L\u01B0u',
  CANCEL: 'H\u1EE7y',
  VIEW: 'Xem',
  OPEN: 'M\u1EDF',
  YES: 'C\xF3',
  NO: 'Kh\xF4ng',
  FREE: 'Mi\u1EC5n ph\xED',
  ALL_LEVELS: 'M\u1ECDi c\u1EA5p \u0111\u1ED9',
  HOURS: 'gi\u1EDD',
  DASH_CRUMB: 'B\u1EA3ng \u0111i\u1EC1u khi\u1EC3n',
};

export const HOME = {
  SLIDE1_KICKER: 'N\u1EC1n t\u1EA3ng h\u1ECDc tr\u1EF1c tuy\u1EBFn h\xE0ng \u0111\u1EA7u',
  SLIDE1_TITLE:
    'H\u1ECDc k\u1EF9 n\u0103ng s\u1EB5n s\xE0ng vi\u1EC7c l\xE0m t\u1EEB kh\xF3a h\u1ECDc tr\u1EF1c tuy\u1EBFn c\xF3 ch\u1EE9ng ch\u1EC9',
  SLIDE1_TEXT:
    'Kh\xE1m ph\xE1 kh\xF3a h\u1ECDc c\xF4ng ngh\u1EC7, kinh doanh, ngh\u1EC7 thu\u1EADt v\xE0 h\u01A1n th\u1EBF n\u1EEFa. B\u1EAFt \u0111\u1EA7u h\u1ECDc ngay!',
  READ_MORE: '\u0110\u1ECDc th\xEAm',
  JOIN_NOW: 'Tham gia ngay',
  SLIDE2_KICKER: 'Ch\xE0o m\u1EEBng \u0111\u1EBFn Secret Coder',
  SLIDE2_TITLE: 'R\xE8n luy\u1EC7n k\u1EF9 n\u0103ng theo nh\u1ECBp c\u1EE7a b\u1EA1n',
  SLIDE2_TEXT: 'N\u1ED9i dung ch\u1EA5t l\u01B0\u1EE3ng, gi\u1EA3ng vi\xEAn chuy\xEAn gia v\xE0 c\u1ED9ng \u0111\u1ED3ng h\u1ED7 tr\u1EE3.',
  BROWSE_COURSES: 'Xem kh\xF3a h\u1ECDc',
  PREV_SLIDE: 'Slide tr\u01B0\u1EDBc',
  NEXT_SLIDE: 'Slide sau',
  GO_SLIDE: (n) => `\u0110\u1EBFn slide ${n}`,
  SECTION_COURSES: 'Kh\xF3a h\u1ECDc',
  POPULAR_COURSES: 'Kh\xF3a h\u1ECDc ph\u1ED5 bi\u1EBFn',
  VIEW_ALL_COURSES: 'Xem t\u1EA5t c\u1EA3 kh\xF3a h\u1ECDc',
  VIEW_ALL_TEAM: 'Xem \u0111\u1EA7y \u0111\u1EE7 \u0111\u1ED9i ng\u0169',
  INSTRUCTOR_TITLE: 'Tr\u1EDF th\xE0nh gi\u1EA3ng vi\xEAn',
  INSTRUCTOR_TEXT:
    'Gi\u1EA3ng vi\xEAn tr\xEAn kh\u1EAFp th\u1EBF gi\u1EDBi \u0111ang d\u1EA1y h\xE0ng tri\u1EC7u h\u1ECDc vi\xEAn. Ch\xFAng t\xF4i cung c\u1EA5p c\xF4ng c\u1EE5 v\xE0 k\u1EF9 n\u0103ng \u0111\u1EC3 b\u1EA1n d\u1EA1y nh\u1EEFng g\xEC m\xECnh y\xEAu th\xEDch.',
  START_TEACHING: 'B\u1EAFt \u0111\u1EA7u gi\u1EA3ng d\u1EA1y',
};

export const ABOUT = {
  TITLE: 'V\u1EC1 ch\xFAng t\xF4i',
  CRUMB: 'Gi\u1EDBi thi\u1EC7u',
  KICKER: 'V\u1EC1 ch\xFAng t\xF4i',
  H2: 'Ch\xE0o m\u1EEBng \u0111\u1EBFn Secret Coder',
  P1:
    'T\u1EA1i Secret Coder, ch\xFAng t\xF4i tin v\xE0o tr\u1EA3i nghi\u1EC7m h\u1ECDc t\u1EADp d\u1EC5 ti\u1EBFp c\u1EADn v\xE0 s\xE1ng t\u1EA1o, ph\xF9 h\u1EE3p l\u1ECBch tr\xECnh v\xE0 phong c\xE1ch c\u1EE7a b\u1EA1n. H\xE3y c\xF9ng ch\xFAng t\xF4i \u0111\xF3n nh\u1EADn t\u01B0\u01A1ng lai gi\xE1o d\u1EE5c v\xE0 ph\xE1t huy ti\u1EC1m n\u0103ng v\u1EDBi c\xE1c kh\xF3a h\u1ECDc tr\u1EF1c tuy\u1EBFn s\xE2u s\u1EAFc.',
  P2:
    'Ch\xE0o m\u1EEBng \u0111\u1EBFn Secret Coder, n\u01A1i h\u1ECDc t\u1EADp kh\xF4ng c\xF3 ranh gi\u1EDBi. S\u1EE9 m\u1EC7nh c\u1EE7a ch\xFAng t\xF4i l\xE0 trao quy\u1EC1n cho m\u1ECDi ng\u01B0\u1EDDi tr\xEAn th\u1EBF gi\u1EDBi th\xF4ng qua gi\xE1o d\u1EE5c d\u1EC5 ti\u1EBFp c\u1EADn v\xE0 \u0111\u1ED5i m\u1EDBi. \u0110i\u1EC1u l\xE0m n\xEAn s\u1EF1 kh\xE1c bi\u1EC7t:',
  VISION: 'T\u1EA7m nh\xECn',
  P_VISION:
    'Secret Coder h\xECnh dung m\u1ED9t th\u1EBF gi\u1EDBi n\u01A1i ai c\u0169ng c\xF3 th\u1EC3 h\u1ECDc, b\u1EA5t k\u1EC3 \u0111\u1ECBa \u0111i\u1EC3m hay ho\xE0n c\u1EA3nh. Ch\xFAng t\xF4i ph\xE1 v\u1EE1 r\xE0o c\u1EA3n \u0111\u1EC3 gi\xE1o d\u1EE5c tr\u1EDF n\xEAn chuy\u1EC3n h\xF3a v\xE0 bao tr\u1EEBm.',
  EXCELLENCE: 'Cam k\u1EBFt ch\u1EA5t l\u01B0\u1EE3ng',
  P_EXCELLENCE:
    'Ch\xFAng t\xF4i t\u1EADp trung v\xE0o gi\xE1o d\u1EE5c \u0111\u1EA1t chu\u1EA9n cao. \u0110\u1ED9i ng\u0169 ph\u1ED1i h\u1EE3p chuy\xEAn gia ng\xE0nh v\xE0 nh\xE0 gi\xE1o \u0111\u1EC3 x\xE2y d\u1EF1ng kh\xF3a h\u1ECDc c\u1EADp nh\u1EADt, mang l\u1EA1i gi\xE1 tr\u1ECB th\u1EF1c cho ng\u01B0\u1EDDi h\u1ECDc.',
  EMPOWER: 'Trao quy\u1EC1n cho ng\u01B0\u1EDDi h\u1ECDc',
  P_EMPOWER:
    'Gi\xE1o d\u1EE5c c\xF3 s\u1EE9c thay \u0111\u1ED5i. Secret Coder gi\xFAp m\u1ECDi ng\u01B0\u1EDDi theo \u0111u\u1ED5i \u0111am m\xEA, th\u0103ng ti\u1EBFn s\u1EF1 nghi\u1EC7p v\xE0 h\u1ECDc k\u1EF9 n\u0103ng m\u1EDBi trong m\xF4i tr\u01B0\u1EDDng n\u0103ng \u0111\u1ED9ng, h\u1ED7 tr\u1EE3.',
  INNOVATION: '\u0110\u1ED5i m\u1EDBi trong h\u1ECDc t\u1EADp',
  P_INNOVATION:
    '\u1EE8ng d\u1EE5ng c\xF4ng ngh\u1EC7, ch\xFAng t\xF4i mang \u0111\u1EBFn ph\u01B0\u01A1ng ph\xE1p v\xE0 c\xF4ng c\u1EE5 h\u1ECDc t\u1EADp m\u1EDBi. T\u1EEB h\u1ECDc ph\u1EA7n t\u01B0\u01A1ng t\xE1c \u0111\u1EBFn bu\u1ED5i tr\u1EF1c ti\u1EBFp, ch\xFAng t\xF4i t\u1EA1o tr\u1EA3i nghi\u1EC7m h\u1EA5p d\u1EABn v\xE0 gi\xFAp ghi nh\u1EDB ki\u1EBFn th\u1EE9c.',
  COMMUNITY: 'H\u01B0\u1EDBng \u0111\u1EBFn c\u1ED9ng \u0111\u1ED3ng',
  P_COMMUNITY:
    'Secret Coder kh\xF4ng ch\u1EC9 l\xE0 kh\xF3a h\u1ECDc m\xE0 c\xF2n l\xE0 c\u1ED9ng \u0111\u1ED3ng s\xF4i \u0111\u1ED9ng. Ch\xFAng t\xF4i khuy\u1EBFn kh\xEDch h\u1EE3p t\xE1c, th\u1EA3o lu\u1EADn v\xE0 chia s\u1EBB ki\u1EBFn th\u1EE9c.',
  DIVERSE: 'Gi\xE1o d\u1EE5c \u0111a d\u1EA1ng v\xE0 h\xF2a nh\u1EADp',
  P_DIVERSE:
    'Ch\xFAng t\xF4i tr\xE2n tr\u1ECDng \u0111a d\u1EA1ng quan \u0111i\u1EC3m, v\u0103n h\xF3a v\xE0 \xFD t\u01B0\u1EDFng. Danh m\u1EE5c kh\xF3a h\u1ECDc phong ph\xFA ph\xF9 h\u1EE3p nhi\u1EC1u s\u1EDF th\xEDch v\xE0 tr\xECnh \u0111\u1ED9.',
  IMPROVE: 'C\u1EA3i ti\u1EBFn li\xEAn t\u1EE5c',
  P_IMPROVE:
    'Ch\xFAng t\xF4i kh\xF4ng ng\u1EEBng ph\xE1t tri\u1EC3n. Ph\u1EA3n h\u1ED3i t\u1EEB h\u1ECDc vi\xEAn gi\xFAp n\xE2ng c\u1EA5p n\u1EC1n t\u1EA3ng, linh ho\u1EA1t v\xE0 s\xE1t nhu c\u1EA7u ng\u01B0\u1EDDi d\xF9ng.',
  CLOSING:
    'C\u1EA3m \u01A1n b\u1EA1n \u0111\xE3 l\xE0 m\u1ED9t ph\u1EA7n c\u1EE7a Secret Coder. C\xF9ng nhau, h\xE3y b\u01B0\u1EDBc tr\xEAn h\xE0nh tr\xECnh h\u1ECDc t\u1EADp su\u1ED1t \u0111\u1EDDi.',
};

export const COURSES_PAGE = {
  TITLE: 'Kh\xF3a h\u1ECDc',
  CRUMB: 'Kh\xF3a h\u1ECDc',
  CATEGORIES: 'Danh m\u1EE5c',
  TOPICS: 'Ch\u1EE7 \u0111\u1EC1 ph\u1ED5 bi\u1EBFn',
  ALL_COURSES: 'T\u1EA5t c\u1EA3 kh\xF3a h\u1ECDc',
  EMPTY: 'Ch\u01B0a c\xF3 kh\xF3a h\u1ECDc \u0111\u01B0\u1EE3c xu\u1EA5t b\u1EA3n. H\xE3y seed c\u01A1 s\u1EDF d\u1EEF li\u1EC7u ho\u1EB7c th\xEAm t\u1EEB qu\u1EA3n tr\u1ECB.',
};

export const COURSE_DETAIL = {
  TITLE_FALLBACK: 'Kh\xF3a h\u1ECDc',
  CRUMB: 'Kh\xF3a h\u1ECDc',
  BACK: 'Quay l\u1EA1i danh s\xE1ch kh\xF3a h\u1ECDc',
  NO_DESC: 'Ch\u01B0a c\xF3 m\xF4 t\u1EA3.',
  LEVEL: 'C\u1EA5p \u0111\u1ED9',
  DURATION: 'Th\u1EDDi l\u01B0\u1EE3ng',
  ENROLL: '\u0110\u0103ng k\xFD ngay',
  ENROLLING: '\u0110ang \u0111\u0103ng k\xFD\u2026',
  ENROLL_SUCCESS: '\u0110\u0103ng k\xFD th\xE0nh c\xF4ng!',
  LOGIN_STUDENT: 'Vui l\xF2ng \u0111\u0103ng nh\u1EADp v\u1EDBi t\u01B0 c\xE1ch h\u1ECDc vi\xEAn \u0111\u1EC3 \u0111\u0103ng k\xFD.',
  ONLY_STUDENT: 'Ch\u1EC9 h\u1ECDc vi\xEAn m\u1EDBi c\xF3 th\u1EC3 th\u1EF1c hi\u1EC7n thao t\xE1c n\xE0y.',
  LOGIN_LINK: '\u0110\u0103ng nh\u1EADp',
  LOGIN_SUFFIX: ' v\u1EDBi t\u01B0 c\xE1ch h\u1ECDc vi\xEAn \u0111\u1EC3 \u0111\u0103ng k\xFD.',
  SECTION_LECTURES: 'B\xE0i gi\u1EA3ng',
  SECTION_QUIZZES: 'B\xE0i ki\u1EC3m tra',
  NO_LECTURES: 'Ch\u01B0a c\xF3 b\xE0i gi\u1EA3ng n\xE0o.',
  NO_QUIZZES: 'Ch\u01B0a c\xF3 b\xE0i ki\u1EC3m tra n\xE0o.',
  SUBMIT_QUIZ: 'N\u1ED9p b\xE0i',
  SUBMITTING_QUIZ: '\u0110ang ch\u1EA5m\u2026',
  QUIZ_SCORE: '\u0110\xFAng {correct}/{total} c\xE2u ({percent}%).',
  QUIZ_SUBMIT_ERR: 'Kh\xF4ng n\u1ED9p b\xE0i \u0111\u01B0\u1EE3c. Vui l\xF2ng th\u1EED l\u1EA1i.',
  OPEN_VIDEO: 'M\u1EDF video',
};

export const CONTACT_PAGE = {
  TITLE: 'Li\xEAn h\u1EC7',
  CRUMB: 'Li\xEAn h\u1EC7',
  H2: 'Li\xEAn h\u1EC7 v\u1EDBi ch\xFAng t\xF4i',
  H3: 'K\u1EBFt n\u1ED1i',
  INTRO:
    'Ch\xFAng t\xF4i s\u1EB5n s\xE0ng h\u1ED7 tr\u1EE3 c\xE2u h\u1ECFi v\u1EC1 kh\xF3a h\u1ECDc Secret Coder, h\u1EE3p t\xE1c hay h\u1ED7 tr\u1EE3 k\u1EF9 thu\u1EADt.',
  OFFICE: 'V\u0103n ph\xF2ng',
  OFFICE_ADDR: '123 \u0110\u01B0\u1EDDng, Bangalore, Karnataka',
  MOBILE: 'Di \u0111\u1ED9ng',
  YOUR_NAME: 'H\u1ECD t\xEAn',
  SUBJECT: 'Ti\xEAu \u0111\u1EC1',
  MESSAGE: 'N\u1ED9i dung',
  SENT_OK: '\u0110\xE3 g\u1EEDi tin nh\u1EAFn. Ch\xFAng t\xF4i s\u1EBD ph\u1EA3n h\u1ED3i s\u1EDBm.',
  SENDING: '\u0110ang g\u1EEDi\u2026',
  SEND: 'G\u1EEDi tin nh\u1EAFn',
};

export const TEAM_PAGE = {
  TITLE: '\u0110\u1ED9i ng\u0169',
  CRUMB: '\u0110\u1ED9i ng\u0169',
  KICKER: '\u0110\u1ED9i ng\u0169',
  H2: 'G\u1EB7p g\u1EE1 gi\u1EA3ng vi\xEAn',
  EMPTY: 'Ch\u01B0a c\xF3 th\xE0nh vi\xEAn. Th\xEAm t\u1EEB b\u1EA3ng qu\u1EA3n tr\u1ECB.',
};

export const TESTI_PAGE = {
  TITLE: '\u0110\xE1nh gi\xE1',
  CRUMB: '\u0110\xE1nh gi\xE1',
  KICKER: '\u0110\xE1nh gi\xE1',
  H2: 'H\u1ECDc vi\xEAn n\xF3i g\xEC',
  EMPTY: 'Ch\u01B0a c\xF3 \u0111\xE1nh gi\xE1.',
};

export const INSTRUCTOR_PAGE = {
  TITLE: 'H\u1EE3p t\xE1c \u0111\xE0o t\u1EA1o',
  CRUMB: 'H\u1EE3p t\xE1c',
  H2: 'Chia s\u1EBB ki\u1EBFn th\u1EE9c c\xF9ng Secret Coder',
  H3: 'Ch\xE0o m\u1EEBng \u0111\u1EBFn Secret Coder',
  P1:
    'B\u1EA1n \u0111am m\xEA chia s\u1EBB chuy\xEAn m\xF4n v\xE0 t\u1EA1o t\xE1c \u0111\u1ED9ng cho h\u1ECDc vi\xEAn kh\u1EAFp n\u01A1i? H\xE3y gia nh\u1EADp c\u1ED9ng \u0111\u1ED3ng nh\xE0 gi\xE1o v\xEC s\u1EF1 y\xEAu th\xEDch h\u1ECDc t\u1EADp.',
  P2:
    'B\u1EAFt \u0111\u1EA7u b\u1EB1ng c\xE1ch t\u1EA1o t\xE0i kho\u1EA3n h\u1ECDc vi\xEAn. N\u1ED9i dung v\xE0 kh\xF3a h\u1ECDc do qu\u1EA3n tr\u1ECB vi\xEAn qu\u1EA3n l\xFD; li\xEAn h\u1EC7 \u0111\u1ED9i ng\u0169 n\u1EBFu b\u1EA1n mu\u1ED1n \u0111\u1ED3ng h\xE0nh gi\u1EA3ng d\u1EA1y.',
  CTA: 'T\u1EA1o t\xE0i kho\u1EA3n',
  WHY: 'V\xEC sao gi\u1EA3ng d\u1EA1y c\xF9ng ch\xFAng t\xF4i?',
  WHY_P:
    'Ti\u1EBFp c\u1EADn h\u1ECDc vi\xEAn to\xE0n c\u1EA7u v\xE0 \u0111\u1ED3ng h\xE0nh c\xF9ng qu\u1EA3n tr\u1ECB \u0111\u1EC3 x\xE2y n\u1ED9i dung ch\u1EA5t l\u01B0\u1EE3ng tr\xEAn n\u1EC1n t\u1EA3ng c\u1EE7a ch\xFAng t\xF4i.',
};

export const AUTH = {
  LOGIN_TITLE: '\u0110\u0103ng nh\u1EADp',
  LOGIN_CRUMB: '\u0110\u0103ng nh\u1EADp',
  LOGIN_LEAD: 'Nh\u1EADp email v\u00E0 m\u1EADt kh\u1EA9u \u0111\u1EC3 v\u00E0o b\u1EA3ng \u0111i\u1EC1u khi\u1EC3n v\u00E0 ti\u1EBFp t\u1EE5c h\u1ECDc.',
  SIGNUP_TITLE: '\u0110\u0103ng k\xFD',
  SIGNUP_CRUMB: '\u0110\u0103ng k\xFD',
  SIGNUP_LEAD: 'T\u1EA1o t\u00E0i kho\u1EA3n h\u1ECDc vi\u00EAn \u0111\u1EC3 \u0111\u0103ng k\u00FD kh\u00F3a h\u1ECDc v\u00E0 theo d\u00F5i ti\u1EBFn \u0111\u1ED9.',
  PASSWORD_HINT: 'T\u1ED1i thi\u1EC3u 6 k\u00FD t\u1EF1.',
  CREATE_ACCOUNT: 'T\u1EA1o t\xE0i kho\u1EA3n',
  FULL_NAME: 'H\u1ECD v\xE0 t\xEAn',
  NO_ACCOUNT: 'Ch\u01B0a c\xF3 t\xE0i kho\u1EA3n?',
  HAS_ACCOUNT: '\u0110\xE3 c\xF3 t\xE0i kho\u1EA3n?',
  CHECK_EMAIL: 'Ki\u1EC3m tra email \u0111\u1EC3 x\xE1c nh\u1EADn t\xE0i kho\u1EA3n, sau \u0111\xF3 \u0111\u0103ng nh\u1EADp.',
  LOGIN_SUCCESS: '\u0110\u0103ng nh\u1EADp th\xE0nh c\xF4ng.',
  SIGNUP_SUCCESS: '\u0110\u0103ng k\xFD th\xE0nh c\xF4ng.',
};

export const DASH_STUDENT = {
  TITLE: 'B\u1EA3ng h\u1ECDc vi\xEAn',
  MY_COURSES: 'Kh\xF3a h\u1ECDc c\u1EE7a t\xF4i',
  TH_COURSE: 'Kh\xF3a h\u1ECDc',
  TH_ENROLLED: 'Ng\xE0y \u0111\u0103ng k\xFD',
  EMPTY: 'B\u1EA1n ch\u01B0a \u0111\u0103ng k\xFD kh\xF3a h\u1ECDc n\xE0o.',
};

export const DASH_ADMIN = {
  TITLE: 'B\u1EA3ng qu\u1EA3n tr\u1ECB',
  CRUMB: 'Qu\u1EA3n tr\u1ECB',
  COURSE_CREATED: '\u0110\xE3 t\u1EA1o kh\xF3a h\u1ECDc',
  CONFIRM_DEL_CAT: 'X\xF3a danh m\u1EE5c n\xE0y?',
  CONFIRM_DEL_COURSE: 'X\xF3a kh\xF3a h\u1ECDc n\xE0y?',
  CONFIRM_DEL: 'X\xF3a m\u1EE5c n\xE0y?',
  TABS: {
    users: 'Ng\u01B0\u1EDDi d\xF9ng',
    courses: 'Kh\xF3a h\u1ECDc',
  },
  COURSE_CONTENT: 'N\u1ED9i dung kh\xF3a h\u1ECDc',
  PICK_COURSE: 'Ch\u1ECDn kh\xF3a h\u1ECDc',
  SUBTAB_LECTURES: 'B\xE0i gi\u1EA3ng',
  SUBTAB_QUIZZES: 'B\xE0i ki\u1EC3m tra',
  LECTURE_TITLE: 'Ti\xEAu \u0111\u1EC1 b\xE0i gi\u1EA3ng',
  LECTURE_CONTENT: 'N\u1ED9i dung (t\xF9y ch\u1ECDn)',
  LECTURE_VIDEO: 'URL video (t\xF9y ch\u1ECDn)',
  ADD_LECTURE: 'Th\xEAm b\xE0i gi\u1EA3ng',
  LECTURE_BLOCKS: 'Kh\u1ED1i b\xE0i h\u1ECDc',
  LECTURE_BLOCK_TITLE: 'Ti\xEAu \u0111\u1EC1 kh\u1ED1i (t\xF9y ch\u1ECDn)',
  LECTURE_BLOCK_CONTENT: 'N\u1ED9i dung',
  LECTURE_BLOCK_VIDEO: 'URL video',
  ADD_LECTURE_BLOCK: 'Th\xEAm kh\u1ED1i',
  REMOVE_LECTURE_BLOCK: 'X\xF3a kh\u1ED1i',
  LECTURE_BLOCKS_EMPTY: 'Th\xEAm \xEDt nh\u1EA5t m\u1ED9t kh\u1ED1i c\xF3 ti\xEAu \u0111\u1EC1, n\u1ED9i dung ho\u1EB7c video.',
  TH_LECTURE_BLOCKS: 'S\u1ED1 kh\u1ED1i',
  QUIZ_TITLE: 'Ti\xEAu \u0111\u1EC1 b\xE0i ki\u1EC3m tra',
  QUIZ_DESC: 'M\xF4 t\u1EA3',
  QUIZ_QUESTIONS_SECTION: 'C\xE1c c\xE2u h\u1ECFi',
  QUIZ_QUESTION_TEXT: 'N\u1ED9i dung c\xE2u',
  QUIZ_OPTION_A: '\u0110\xE1p \xE1n A',
  QUIZ_OPTION_B: '\u0110\xE1p \xE1n B',
  QUIZ_OPTION_C: '\u0110\xE1p \xE1n C',
  QUIZ_OPTION_D: '\u0110\xE1p \xE1n D',
  QUIZ_CORRECT: '\u0110\xE1p \xE1n \u0111\xFAng',
  ADD_QUIZ_QUESTION: 'Th\xEAm c\xE2u h\u1ECFi',
  REMOVE_QUIZ_QUESTION: 'X\xF3a c\xE2u',
  QUIZ_OPTIONS_REQUIRED: 'M\u1ED7i c\xE2u c\u1EA7n \u0111\u1EE7 4 \u0111\xE1p \xE1n A\u2013D.',
  QUIZ_NEED_QUESTION: 'Th\xEAm \xEDt nh\u1EA5t m\u1ED9t c\xE2u h\u1ECFi h\u1EE3p l\u1EC7.',
  ADD_QUIZ: 'Th\xEAm b\xE0i ki\u1EC3m tra',
  TOAST_LECTURE_ADDED: '\u0110\xE3 th\xEAm b\xE0i gi\u1EA3ng',
  TOAST_QUIZ_ADDED: '\u0110\xE3 th\xEAm b\xE0i ki\u1EC3m tra',
  CONFIRM_DEL_LECTURE: 'X\xF3a b\xE0i gi\u1EA3ng n\xE0y?',
  CONFIRM_DEL_QUIZ: 'X\xF3a b\xE0i ki\u1EC3m tra n\xE0y?',
  TH_LECTURE_TITLE: 'B\xE0i gi\u1EA3ng',
  TH_QUIZ_TITLE: 'B\xE0i ki\u1EC3m tra',
  TH_NAME: 'T\xEAn',
  TH_ROLE: 'Vai tr\xF2',
  TH_ACTIONS: 'Thao t\xE1c',
  BTN_ADD_USER: 'Th\xEAm ng\u01B0\u1EDDi d\xF9ng',
  BTN_EDIT: 'S\u1EEDa',
  USER_DIALOG_CREATE: 'Th\xEAm ng\u01B0\u1EDDi d\xF9ng',
  USER_DIALOG_EDIT: 'S\u1EEDa ng\u01B0\u1EDDi d\xF9ng',
  USER_CREATED: '\u0110\xE3 t\u1EA1o ng\u01B0\u1EDDi d\xF9ng',
  USER_UPDATED: '\u0110\xE3 c\u1EADp nh\u1EADt ng\u01B0\u1EDDi d\xF9ng',
  CONFIRM_DEL_USER:
    'X\xF3a t\xE0i kho\u1EA3n n\xE0y? Kh\xF4ng th\u1EC3 ho\xE0n t\xE1c.',
  CANNOT_DELETE_SELF: 'Kh\xF4ng th\u1EC3 x\xF3a ch\xEDnh t\xE0i kho\u1EA3n \u0111ang \u0111\u0103ng nh\u1EADp.',
  PASSWORD_REQUIRED: 'Vui l\xF2ng nh\u1EADp m\u1EADt kh\u1EA9u.',
  EMAIL_REQUIRED: 'Vui l\xF2ng nh\u1EADp email.',
  PW_LEAVE_BLANK: '\u0110\u1EC3 tr\u1ED1ng n\u1EBFu kh\xF4ng \u0111\u1ED5i m\u1EADt kh\u1EA9u',
  ROLE_STUDENT: 'H\u1ECDc vi\xEAn',
  ROLE_ADMIN: 'Qu\u1EA3n tr\u1ECB',
  TH_DATE: 'Ng\xE0y',
  TH_EMAIL: 'Email',
  TH_SUBJECT: 'Ti\xEAu \u0111\u1EC1',
  TH_MESSAGE: 'N\u1ED9i dung',
  PH_NAME: 'T\xEAn',
  PH_SLUG: 'slug',
  PH_IMAGE_URL: 'URL \u1EA3nh',
  ADD_COURSE: 'Th\xEAm kh\xF3a h\u1ECDc',
  LABEL_COURSE_TITLE: 'Ti\xEAu \u0111\u1EC1',
  LABEL_COURSE_DESC: 'M\xF4 t\u1EA3',
  LABEL_CATEGORY: 'Danh m\u1EE5c',
  LABEL_THUMB: 'URL \u1EA3nh \u0111\u1EA1i di\u1EC7n',
  TH_COURSE_TITLE: 'Ti\xEAu \u0111\u1EC1',
  TH_COURSE_SLUG: 'Slug',
  TH_COURSE_PUBLISHED: 'Xu\u1EA5t b\u1EA3n',
  LABEL_PRICE_CENTS: 'Gi\xE1 (xu / cents)',
  LABEL_DURATION_H: 'Th\u1EDDi l\u01B0\u1EE3ng (gi\u1EDD)',
  LABEL_LEVEL: 'C\u1EA5p \u0111\u1ED9',
  LABEL_PUBLISHED: '\u0110\xE3 xu\u1EA5t b\u1EA3n',
  PH_ROLE: 'Ch\u1EE9c danh',
  PH_BIO: 'Ti\u1EC3u s\u1EED',
  PH_AUTHOR: 'T\xE1c gi\u1EA3',
  PH_TITLE: 'Ch\u1EE9c danh',
  PH_CONTENT: 'N\u1ED9i dung',
  LEVEL_DEFAULT: 'C\u01A1 b\u1EA3n',
  TOAST_CAT_ADDED: '\u0110\xE3 th\xEAm danh m\u1EE5c',
  TOAST_TEAM_ADDED: '\u0110\xE3 th\xEAm th\xE0nh vi\xEAn',
  TOAST_TEST_ADDED: '\u0110\xE3 th\xEAm \u0111\xE1nh gi\xE1',
  TOAST_DELETED: '\u0110\xE3 x\xF3a',
};
